...
describe("TroubleshootingCategoryPage", () => {
  ...
  beforeEach(() => {
    ...
    wrapper = shallow(<TroubleshootingCategoryPage type="diagnostics" />, {
      context,
    });
  });

  it("renders a tab container", () => {
    const container = wrapper.find(DiagnosticsTabContainer);
    expect(container.length).to.equal(1);
  });

  it("switches tabs", () => {
    expect(wrapper.state()["tab"]).to.equal("coverage_provider");
    expect(wrapper.find(DiagnosticsTabContainer).prop("tab")).to.equal(
      "coverage_provider"
    );
