...
describe("EditorField", () => {
  ...
  it("renders buttons", () => {
    const stubChangeStyle = stub(wrapper.instance(), "changeStyle");
    const buttons = wrapper.find("button");
    expect(buttons.length).to.equal(3);
    expect(stubChangeStyle.callCount).to.equal(0);

    const bold = buttons.at(0);
  ...
  it("gets the default content if no content is an empty html string", () => {
    ...
    expect(wrapper.instance().getValue()).to.equal(
      "<p>Update the content for this field.</p>"
    );
