...
describe("Stats", () => {
  ...
  describe("rendering", () => {
    ...
    it("shows stats data", () => {
      ...
      expect(groups.at(0).text()).to.contain("Total Patrons:3456");
      expect(groups.at(0).text()).to.contain("Patrons with Active Loans:55");
      expect(groups.at(0).text()).to.contain("Patrons with Active Loans or Holds:1234");
      expect(groups.at(0).text()).to.contain("Loans:100");
      expect(groups.at(0).text()).to.contain("Holds:2000");

      expect(groups.at(1).text()).to.contain("Inventory");
      expect(groups.at(1).text()).to.contain("Titles:54321");
      expect(groups.at(1).text()).to.contain("Licenses:123456");
      ...
      expect(groups.at(2).text()).to.contain("Open Access Titles:1000");
    });
  });
});