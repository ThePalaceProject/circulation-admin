...
export default function CustomListEditor({
  ...
  const hasChanges = (): boolean => {
    let titleChanged = list && list.title !== listTitle;
    let entriesChanged =
      list && !!list.books && list.books.length !== entries.length;
    // If the current list is new then props.list will be undefined, but
    // the state for the entries or title could be populated, so there's a need to check.
    if (!list) {
      titleChanged = listTitle && listTitle !== "";
      entriesChanged = entries?.length > 0;
    }
    ...
    if (!entriesChanged) {
      const propsIds = ((list && list.books) || [])
        .map((entry) => entry.id)
        .sort();
      const stateIds = entries?.map((entry) => entry.id).sort();
    ...
    if (listCollections && listCollections.length !== collections.length) {
      collectionsChanged = true;
    } else {
      const propsIds = (listCollections || [])
        .map((collection) => collection.id)
        .sort();
      const stateIds = collectionsOnState
        .map((collection) => collection.id)
        .sort();
      for (let i = 0; i < propsIds.length; i++) {
        if (propsIds[i] !== stateIds[i]) {
  ...
  const reset = () => {
    ...
    setTimeout(() => {
      ...
      setEntries(list ? list.books : []);
      setCollectionsOnState(listCollections || []);
      setEntryPointSelected("all");
    }, 200);
  };
