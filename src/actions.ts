...
export default class ActionCreator extends BaseActionCreator {
  ...
  fetchPlugins() {
    ...
    return this.fetchJSON<PluginListData>(ActionCreator.PLUGINS, url).bind(
      this
    );
  }

  fetchPlugin(library: string, plugin: string) {
    const url = "/admin/plugin/" + library + "/" + plugin;
    return this.fetchJSON<PluginListData>(ActionCreator.PLUGIN, url).bind(this);
  }
