...
describe("editorAdapter", () => {
  it("adapts valid OPDS entry", () => {
    ...
    expect(adapted.authors).toStrictEqual([
      { name: "name", uri: "uri", role: "aut" },
    ]);
    // The adapter flattens `Contributor` instances into plain objects, so assert
    // the shape it produces rather than comparing against `entry.contributors`.
    expect(adapted.contributors).toStrictEqual([
      { name: "name2", uri: "uri2", role: "role2" },
    ]);
    expect(adapted.subtitle).toBe("subtitle");
    expect(adapted.summary).toBe("content");
    expect(adapted.audience).toBe("ya");
    expect(adapted.targetAgeRange).toStrictEqual(["13", "16"]);
    expect(adapted.fiction).toBe(true);
    expect(adapted.categories).toStrictEqual(["label", "age", "ya", "Fiction"]);
    expect(adapted.hideLink).toStrictEqual(entry.links[0]);
    expect(adapted.restoreLink).toStrictEqual(entry.links[1]);
    expect(adapted.refreshLink).toStrictEqual(entry.links[2]);
    expect(adapted.editLink).toStrictEqual(entry.links[3]);
    expect(adapted.issuesLink).toStrictEqual(entry.links[4]);
    expect(adapted.changeCoverLink).toStrictEqual(entry.links[5]);
