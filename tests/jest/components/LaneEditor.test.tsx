...
// (`src/components/__tests__/LaneEditor-test.tsx`). Where the legacy test reached
// into class internals (stubbing `TextWithEditMode.prototype.getText` /
// `LaneCustomListsEditor.prototype`), these drive the real name field and the
// mocked custom-lists editor's callbacks instead, asserting on the resulting DOM
// and on the props passed to the injected `editLane` / `deleteLane` /
// `toggleLaneVisibility` handlers. Navigation is asserted by spying on
// `navigate.navigateTo`.
describe("LaneEditor — legacy behaviors", () => {
  const twoLists = [
    { id: 1, name: "list 1", entries: [], is_owner: true, is_shared: false },
