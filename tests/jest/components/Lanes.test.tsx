...
// ------------------------------
// `Lanes` reorders lanes with `react-beautiful-dnd`, and a real drag/drop cannot
// be simulated in jsdom. The outcomes of a drag are still covered:
//   - `orderChanged` (and so the "Change Lane Order" panel), saving the reordered
//     tree via `changeLaneOrder`, and cancelling it via `resetOrder` are all
//     exercised in the "Lanes - reorder flow" describe below, which calls
//     `Lanes.drag(...)` through a ref — the same callback LanesSidebar invokes on
//     a real drop.
//   - The drop boundary (a lane may only be dropped within its own parent) lives
//     in LanesSidebar and is asserted in LanesSidebar.test.tsx.
// Only the pointer-level gestures themselves ("drags a top-level lane", "drags a
// sublane") have no jsdom-reachable trigger; the tree structure and expand/collapse
// behavior they produce are asserted instead (see "renders the draggable lane tree"
// and "expands and collapses lanes").
//
...
// The lane-reorder flow. `Lanes.drag` is the callback LanesSidebar invokes on a
// real drop (which jsdom can't perform), so we call it through a ref to drive the
// order-changed UI, then exercise save/reset and findParentOfLane.
describe("Lanes - reorder flow", () => {
  ...
  it("shows the reorder UI after a drag, then saves the reordered lanes and resets", async () => {
    const user = userEvent.setup();
    const changeLaneOrder = jest.fn().mockResolvedValue(undefined);
    const fetchLanes = jest.fn();
    const ref = React.createRef<any>();
    // Two top-level lanes, so a reorder is observable in the saved payload.
    const secondLane: LaneData = {
      id: 4,
      display_name: "lane 4",
      visible: true,
      count: 1,
    ...
    const initialLanes = [...makeLanesData(), secondLane];
    const { container } = render(
      <Lanes
        ref={ref}
        {...props({ changeLaneOrder, fetchLanes, lanes: initialLanes })}
      />
    );

    // A drop hands the reordered tree to Lanes.drag (what LanesSidebar does),
    // which stores it and flags orderChanged.
    const originalOrder = ref.current.state.lanes;
    const reordered = [...originalOrder].reverse();
    act(() => ref.current.drag({ lanes: reordered, orderChanged: true }));
    ...
    // The exact reordered tree is persisted — not the original order.
    expect(changeLaneOrder).toHaveBeenCalledTimes(1);
    expect(changeLaneOrder).toHaveBeenCalledWith(reordered);
    expect(changeLaneOrder.mock.calls[0][0]).not.toEqual(originalOrder);
    // saveOrder() only fetches once changeLaneOrder resolves.
    await waitFor(() => expect(fetchLanes).toHaveBeenCalledTimes(1));

    // With a reorder pending in state, cancelling runs resetOrder(), which
    // restores state.lanes from props and clears the panel.
    act(() => ref.current.drag({ lanes: reordered, orderChanged: true }));
    expect(ref.current.state.lanes).toEqual(reordered);

    await user.click(
      container.querySelector("button.cancel-lane-order-changes") as HTMLElement
    );

    expect(ref.current.state.lanes).toEqual(initialLanes);
    expect(
