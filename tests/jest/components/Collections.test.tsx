...
// ── Migrated from src/components/__tests__/Collections-test.tsx ───────────────
//
// These exercise coverage the legacy enzyme test provided that the disclosure
// tests above do not: the connect() wiring (mapStateToProps / mapDispatchToProps,
// which only run when the CONNECTED default export mounts), the
// marked-for-deletion list item, the confirm-before-disassociating-libraries
// behavior, the getChildContext().registerLibrary closure, the on-mount
// registrations fetch, and the delete flow.

describe("Collections - connected wiring, deletion, and registration (migrated)", () => {
  const allLibraries = [
