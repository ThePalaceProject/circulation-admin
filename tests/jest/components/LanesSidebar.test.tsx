...
describe("LanesSidebar", () => {
  ...
  it("disallows dropping a lane outside its own parent", () => {
    // The dragging lane's parent (id 999) differs from every target lane, so the
    // component marks each Droppable it renders as isDropDisabled — including the
    // top-level list. Assert the flag the component actually passed to react-
    // beautiful-dnd, not just that the sidebar rendered.
    renderSidebar({
      findLaneForIdentifier: jest.fn().mockReturnValue(makeLanes()[0]),
      findParentOfLane: jest.fn().mockReturnValue({ id: 999 } as LaneData),
    });

    const droppables = getHandlers().droppables;
    expect(droppables.length).toBeGreaterThan(0);
    expect(droppables.every((d: any) => d.isDropDisabled)).toBe(true);
    expect(
      droppables.find((d: any) => d.droppableId === "top")?.isDropDisabled
    ).toBe(true);
  });
});
