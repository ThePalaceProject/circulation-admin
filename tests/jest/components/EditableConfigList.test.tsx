...
// (src/components/__tests__/EditableConfigList-test.tsx). These exercise the
// abstract base class through concrete test subclasses, mirroring the legacy
// approach but asserting observable DOM instead of reaching into instances.
describe("EditableConfigList - legacy behaviors", () => {
  interface Thing {
    id: number;
    label: string;
    level?: number;
  ...
  // Closure-controlled permission flags, mirroring the legacy test's design.
  let canCreate: boolean;
  let canDelete: boolean;
  let canEdit: boolean;

  // A test EditForm that surfaces the props the base class passes to it, so
  // prop-only legacy assertions can be checked against the rendered DOM.
  ...
  // Overrides permission checks from the closure flags, like the legacy test.
